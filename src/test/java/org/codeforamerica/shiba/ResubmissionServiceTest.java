...
class ResubmissionServiceTest {
    ...
    void setUp() {
        countyMap.setDefaultValue(MnitCountyInformation.builder()
                ...
                .email(defaultEmail)
                .build());
        resubmissionService = new ResubmissionService(applicationRepository, emailClient, countyMap, pdfGenerator, combinedDocumentRepositoryService);
    }
    ...
    void itResubmitsCCAPAndUploadedDocuments() {
        ...
        applicationData.addUploadedDoc(new MockMultipartFile("image", "test".getBytes()), "someS3FilePath", "someDataUrl", "image/jpeg");
        var application = Application.builder().id(appId).county(Olmsted).applicationData(applicationData).build();
        when(applicationRepository.getApplicationIdsToResubmit()).thenReturn(Map.of(CCAP, List.of(appId), UPLOADED_DOC, List.of(appId)));
        when(applicationRepository.find(appId)).thenReturn(application);

        var uploadedDocWithCoverPageFile = new ApplicationFile("test".getBytes(), "fileName.txt");
        var coverPage = "someCoverPageText".getBytes();
        when(pdfGenerator.generate(application, UPLOADED_DOC, Recipient.CASEWORKER)).thenReturn(new ApplicationFile(coverPage, "coverPage"));
        when(pdfGenerator.generateForUploadedDocument(applicationData.getUploadedDocs().get(0), 0, application, coverPage)).thenReturn(uploadedDocWithCoverPageFile);
        ...
        verify(emailClient, times(2)).resubmitFailedEmail(eq(defaultEmail), documentCaptor.capture(), applicationFileCaptor.capture(), eq(application), eq(ENGLISH));
        assertThat(applicationFileCaptor.getAllValues()).containsExactlyInAnyOrder(uploadedDocWithCoverPageFile, ccapFile);
        ...
        verify(applicationRepository, times(2)).updateStatus(eq(appId), applicationRepositoryDocumentCaptor.capture(), eq(Status.DELIVERED));
        assertThat(applicationRepositoryDocumentCaptor.getAllValues()).containsExactlyInAnyOrder(UPLOADED_DOC, CCAP);
    }
    ...
    void itUpdatesTheStatusWhenResubmissionIsUnsuccessful() {
        ...
        var coverPage = "someCoverPageText".getBytes();
        when(pdfGenerator.generate(application, UPLOADED_DOC, Recipient.CASEWORKER)).thenReturn(new ApplicationFile(coverPage, "coverPage"));
        when(pdfGenerator.generateForUploadedDocument(applicationData.getUploadedDocs().get(0), 0, application, coverPage)).thenThrow(RuntimeException.class);

        resubmissionService.resubmitFailedApplications();

        verify(emailClient, never()).resubmitFailedEmail(defaultEmail, UPLOADED_DOC, uploadedDocWithCoverPageFile, application, ENGLISH);
