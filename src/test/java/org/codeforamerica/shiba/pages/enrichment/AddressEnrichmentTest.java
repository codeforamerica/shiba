...
class AddressEnrichmentTest {
  ...
  void shouldCallAddressValidationOnAddressValidationPage() {
    ...
    assertThat(enrichmentResult).containsEntry("enrichedStreetAddress",
        new InputData(List.of(expectedAddress.getStreet())));
    assertThat(enrichmentResult)
        .containsEntry("enrichedCity", new InputData(List.of(expectedAddress.getCity())));
    assertThat(enrichmentResult)
        .containsEntry("enrichedState", new InputData(List.of(expectedAddress.getState())));
    assertThat(enrichmentResult)
        .containsEntry("enrichedZipCode", new InputData(List.of(expectedAddress.getZipcode())));
    assertThat(enrichmentResult).containsEntry("enrichedApartmentNumber",
        new InputData(List.of(expectedAddress.getApartmentNumber())));
    assertThat(enrichmentResult)
        .containsEntry("enrichedCounty", new InputData(List.of(expectedAddress.getCounty())));
  }
  ...
  void shouldIncludeCountyFromMapping_whenLocationClientDoesNotReturnAnAddress() {
    ...
    assertThat(enrichmentResult)
        .containsOnly(Map.entry("enrichedCounty", new InputData(List.of("Olmsted"))));
  }
