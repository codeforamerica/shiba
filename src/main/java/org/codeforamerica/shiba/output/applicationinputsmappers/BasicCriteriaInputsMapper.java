...
public class BasicCriteriaInputsMapper extends OneToManyApplicationInputsMapper {
  ...
  protected List<ApplicationInput> map(PagesData pagesData) {
    ...
    boolean blindOrHasDisability = basicCriteria.stream()
        .anyMatch(BLIND_OR_HAS_DISABILITY::contains);
    result.add(createApplicationInput(blindOrHasDisability, "blindOrHasDisability"));
