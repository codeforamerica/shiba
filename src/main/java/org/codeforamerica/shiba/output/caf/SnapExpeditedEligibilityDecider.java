...
public class SnapExpeditedEligibilityDecider {
    ...
    public SnapExpeditedEligibility decide(ApplicationData applicationData) {
        ...
        Money income = totalIncomeCalculator.calculate(new TotalIncome(last30DaysIncome, jobIncomeInformation));
        ...
        boolean isMigrantWorker = Boolean.parseBoolean(getFirstValue(applicationData.getPagesData(), MIGRANT_WORKER));
    ...
    private boolean isApplyingForSnap(ApplicationData applicationData) {
        boolean applicantApplyingForSnap = getValues(applicationData.getPagesData(), APPLICANT_PROGRAMS).contains("SNAP");
        ...
        boolean isPreparingMealsTogether = Boolean.parseBoolean(getFirstValue(applicationData.getPagesData(), PREPARING_MEALS_TOGETHER));
    ...
    private boolean canDetermineEligibility(ApplicationData applicationData) {
        ...
        boolean missingUtilities = getFirstValue(applicationData.getPagesData(), UTILITY_EXPENSES_SELECTIONS) == null;
