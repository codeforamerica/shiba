...
public class MaskedSerializer extends StdSerializer<PageData> {
  ...
  @Override
  public void serialize(PageData pageData, JsonGenerator gen, SerializerProvider provider)
      throws IOException {
    gen.writeStartObject();
    pageData.keySet().forEach(inputName -> {
      try {
        gen.writeStringField(inputName,
            pageData.get(inputName).getValue().isEmpty() ? "" : "filled");
      } catch (IOException e) {
        e.printStackTrace();
      }
    });
