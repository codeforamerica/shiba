...
public class ApplicationDataEncryptor implements Encryptor<ApplicationData> {
    ...
    private void runCryptographicFunctionOnData(Function<String, String> encryptFunc, ApplicationData applicationData) {
        ...
        if (hasHousehold) {
            applicationData.getSubworkflows().get("household")
                    .forEach(iteration -> setEncryptedSSNValue(encryptFunc, iteration.getPagesData(), "householdMemberInfo"));
        }
    }
     ...
     */
    private void setEncryptedSSNValue(Function<String, String> encryptFunc, PagesData pagesData, String pageName) {
