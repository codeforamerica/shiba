...
public class EsbRetryListener extends RetryListenerSupport {
    ...
    public <T, E extends Throwable> void onError(RetryContext context, RetryCallback<T, E> callback, Throwable throwable) {
        if (context.getRetryCount() < Integer.parseInt(maxAttempts)) {
            log.info("ESB Retry Unsuccessful for: " + MDC.get("applicationFile") + ", trying again. Retries Attempted: {} ", context.getRetryCount());
        } else {
            log.info("ESB Retry Unsuccessful for: " + MDC.get("applicationFile") + ". Retries Attempted: {} ", context.getRetryCount());
        }
